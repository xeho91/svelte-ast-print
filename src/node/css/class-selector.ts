 ...
 */
export const print_css_class_selector = define_printer((node: Css.ClassSelector, options) => {
	return "";
});
...
if (import.meta.vitest) {
	...
	describe("Css.ClassSelector", () => {
		it("prints correctly", ({ expect }) => {
			...
			expect(print_css_class_selector(node, DEFAULT_OPTIONS)).toMatchInlineSnapshot("");
		});
	});
}
